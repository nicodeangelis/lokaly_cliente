-- PHASE 1: Database Foundation for Loyalty QR Epic

-- 1) Unique index to prevent duplicate daily visits
CREATE UNIQUE INDEX IF NOT EXISTS visitas_unique_user_local_day
  ON public.visitas (user_id, local_id, (created_at::date));

-- 2) User preferences table for coffee taste personalization
...
-- 3) Service rating table for post-consumption surveys
...
-- 4) Helper function to refresh user level based on points
...
-- 5) RPC for wall QR onboarding flow
...
BEGIN
  ...
  ON CONFLICT (user_id, local_id, (created_at::date))
...
-- 6) RPC for dynamic QR redemption with service rating
