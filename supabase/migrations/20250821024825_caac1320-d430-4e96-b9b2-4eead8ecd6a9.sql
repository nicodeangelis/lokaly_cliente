...
SET imagen = CASE 
  ...
  ELSE imagen
END
WHERE local_id IN (SELECT id FROM public.locales WHERE nombre LIKE '%Arregui%');
...
SET imagen = CASE 
  ...
  ELSE imagen
END
WHERE local_id IN (SELECT id FROM public.locales WHERE nombre LIKE '%Mendoza%');

-- Add more detailed descriptions and ingredients to menu items for Arregui
UPDATE public.menu_items 
SET 
  descripcion = CASE 
